...
def create_agent_settings_tab(webui_manager: WebuiManager):
    """Minimal agent settings – only what matters for self‑hosted Ollama."""
    tab_components = {}
    ...
    with gr.Group():
        llm_temperature = gr.Slider(minimum=0.0, maximum=2.0, value=0.6, step=0.1,
                                    label="Temperature", info="Controls randomness", interactive=True)
        use_vision = gr.Checkbox(label="Use Vision", value=True, info="Send screenshot to LLM", interactive=True)
        ollama_num_ctx = gr.Slider(minimum=256, maximum=32768, value=16000, step=256,
                                   label="Context Length", info="Higher = slower, more memory", interactive=True)

    with gr.Group():
        max_steps = gr.Slider(minimum=1, maximum=1000, value=100, step=1,
                              label="Max Run Steps", interactive=True)
        max_actions = gr.Slider(minimum=1, maximum=100, value=10, step=1,
                                label="Max Actions per Step", interactive=True)
